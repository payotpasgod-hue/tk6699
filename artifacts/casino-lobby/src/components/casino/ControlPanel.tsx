...
export function ControlPanel() {
  ...
  return (
    <div className="glass-panel p-6 rounded-2xl mb-8 flex flex-col lg:flex-row gap-6">
      
      {/* Player Management Controls */}
      <div className="flex-1 flex flex-col gap-4 border-b lg:border-b-0 lg:border-r border-white/10 pb-6 lg:pb-0 lg:pr-6">
        <h3 className="text-lg font-display font-semibold text-white flex items-center gap-2">
          <Coins className="w-5 h-5 text-accent" /> Wallet Operations
        </h3>
        <div className="flex flex-wrap gap-3">
          ...
          <Dialog open={isPlayerDialogOpen} onOpenChange={setPlayerDialogOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" className="bg-white/5 border-white/10 hover:bg-white/10">
                <Plus className="w-4 h-4 mr-2" /> New Player
              </Button>
            </DialogTrigger>
          ...
          <Dialog open={isDepositDialogOpen} onOpenChange={setDepositDialogOpen}>
            <DialogTrigger asChild>
              <Button className="bg-secondary text-secondary-foreground hover:bg-secondary/80">
                <Send className="w-4 h-4 mr-2" /> Seed Balance
              </Button>
            </DialogTrigger>
            <DialogContent className="bg-card border-white/10 text-white">
              <DialogHeader>
          ...
          <Button variant="destructive" onClick={handleWithdrawAll} disabled={isWithdrawing || store.balance <= 0}>
            <Download className="w-4 h-4 mr-2" /> Withdraw All
          </Button>
        </div>
      </div>
      ...
      <div className="flex-[2] flex flex-col gap-4">
        <h3 className="text-lg font-display font-semibold text-white flex items-center gap-2">
          <Search className="w-5 h-5 text-primary" /> Filter Games
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="relative">
            ...
            <Input 
              ...
              className="pl-10 bg-black/20 border-white/10 focus-visible:ring-primary h-11"
